 ...
 * </pre>
 */
public class MapWithHashing<K, V> extends MapSecondary<K, V> {
    ...
    private static int mod(int a, int b) {
        ...
        // adjust negative numbers for modular
        if (answer < 0) {
            answer = answer + b;
        }
    ...
    private void createNewRep(int hashTableSize) {
        ...
        // add a map element to every index
        int index = 0;
        while (index < hashTableSize) {
            Map<K, V> element = new Map2<>();
            this.hashTable.setEntry(index, element);
            index++;
        }
    ...
    public Map4() {
        ...
        // create default representative single bucket
        this.createNewRep(1);

        // give initial value to size
        this.size = 0;
    ...
    public Map4(int hashTableSize) {
        ...
        // create representation with a set number of buckets
    ...
    public final void add(K key, V value) {
        ...
        // get alias to specific bucket map
        Map<K, V> handle = this.hashTable.entry(mod(key.hashCode(),
                this.hashTable.length()));

        // add key and value via handle
    ...
    public final Pair<K, V> remove(K key) {
        ...
        // remove value by key via handle
        Pair<K, V> answer = handle.remove(key);
    ...
    public final Pair<K, V> removeAny() {
        ...
        boolean check = false;
        //Create random number between 0 and |hashTable|-1
        int randNum = (int) (Math.random() * this.hashTable.length());
        while (!check) {
            //makes sure bucket isn't empty and the map isn't either
            if (this.hashTable.mayBeExamined(randNum)
                    && this.hashTable.entry(randNum).size() > 0) {
                check = true;
            } else {
                randNum = (int) (Math.random() * this.hashTable.length());
            }
        }

        // get alias to specific bucket map
        Map<K, V> handle = this.hashTable.entry(randNum);

        // remove pair from handle
        Pair<K, V> answer = handle.removeAny();
    ...
    public final V value(K key) {
        ...
        // retrieve value via handle
        V extractedVal = handle.value(key);
