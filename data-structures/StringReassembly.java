...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        /*
         * Trim first string to remove trailing overlap and create new string
         * with both input string added together
         */

        String str1Chopped = str1.substring(0, str1.length() - overlap);

        String combination = str1Chopped + str2;
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet, String str) {
        ...
        boolean includesSubstring = false;

        // check if input is a substring of any member in set
        while (strSet.size() != 0) {
            String removedElement = strSet.removeAny();
            if (removedElement.contains(str)) {
                includesSubstring = true;
            }
            temp.add(removedElement);
        }

        /*
         * if input not an existing substring, remove any members that are
         * subset of input and add input
         */
        if (!includesSubstring) {

            while (temp.size() != 0) {
                String removedElement = temp.removeAny();
                if (!str.contains(removedElement)) {
                    strSet.add(removedElement);
                }
            }
            strSet.add(str);
        } else {
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        Set<String> returnSet = new Set1L<>();

        /* add all lines from input into a set while avoid subsets */
        while (!input.atEOS()) {
            String current = input.nextLine();
            addToSetAvoidingSubstrings(returnSet, current);
        }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        /*
         * loop through entire input String and move into output file, replacing
         * '~' character with a line space
         */
        for (int i = 0; i < text.length(); i++) {
            String extract = text.substring(i, i + 1);
            if (extract.equals("~")) {
                out.println("");
            } else {
                out.print(extract);
